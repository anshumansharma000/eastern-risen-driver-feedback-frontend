...
import type { AdminFeedbackAnswer, AdminFeedbackDetail, AdminFeedbackPhotoAccessResponse, AdminFeedbackSummary, AgencySettings, FeedbackReviewState } from "@/lib/contracts";
...
export function AdminFeedbackDetailView({ feedbackId }: { feedbackId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [value, settings] = await Promise.all([
        getData<AdminFeedbackDetail>(`/api/v1/admin/feedback/${encodeURIComponent(feedbackId)}`),
  ...
  return <div className="feedback-detail-page">
    <div className="screen-feedback-detail">
      <button className="button button-secondary feedback-back" onClick={() => router.back()}>← Back to feedback</button>
      <div className="page-header feedback-detail-header">
        <div><p className="eyebrow">Passenger feedback</p><h1>{detail.respondent.name}</h1><p>Booking {detail.bookingReference} · submitted {formatInTimezone(detail.submittedAt, timezone)}</p></div>
    ...
    <PrintReport detail={detail} answers={answers} timezone={timezone} mode={printMode} />
...
function AnswerList({ answers, print = false }: { answers: AdminFeedbackAnswer[]; print?: boolean }) {
  return <section className={print ? "print-answers" : "card card-pad feedback-answers"}><h2 className="section-title">Feedback answers</h2>{!print && <p className="trip-meta">Clear responses to each question, in questionnaire order.</p>}<div className="answer-list">{answers.map((answer, index) => <article key={answer.id} className="answer-row"><header><span className="answer-number">{index + 1}</span><span className="answer-category">{categoryLabel(answer.category)}</span>{answer.numericScore !== null && <strong className="answer-score">Score: {answer.numericScore.toFixed(2)}</strong>}</header><h3>{answer.prompt}</h3><div className="answer-response"><span className="answer-response-label">Response</span><div className="answer-value"><HumanAnswerValue value={answer.value} questionType={answer.questionType} /></div></div></article>)}</div></section>;
}
...
function PrintReport({ detail, answers, timezone, mode }: { detail: AdminFeedbackDetail; answers: AdminFeedbackAnswer[]; timezone: string; mode: PrintMode }) {
  const userCopy = mode === "USER";
  return <article className="print-feedback-report" aria-hidden="true">
    <header className="print-report-header"><p>{userCopy ? "User copy" : "Agent copy"}</p><h1>{userCopy ? detail.respondent.name : COMPANY_NAME}</h1><h2>Passenger feedback report</h2><span>{userCopy ? `Issued by ${COMPANY_NAME}` : `Prepared for ${detail.respondent.name}`}</span></header>
