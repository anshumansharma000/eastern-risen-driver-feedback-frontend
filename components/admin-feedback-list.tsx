...
export function AdminFeedbackList(){
  ...
  return <><div className="page-header"><div><p className="eyebrow">Immutable passenger records</p><h1>Feedback review</h1><p>Filter by submission date and operational dimensions. Passenger contact details remain confined to the protected detail page.</p></div><button className="button button-secondary" disabled={loading} onClick={()=>void load()}>Refresh</button></div>
    ...
    {response&&response.data.length>0&&<><p className="trip-meta feedback-date-basis">Times use {response.meta.timezone}; month filters use submission date.</p><div className="feedback-table-wrap"><table className="feedback-table"><thead><tr><th>Booking / respondent</th><th>Driver</th><th>Submitted</th><th>Mode</th><th>Review</th><th>Score</th></tr></thead><tbody>{response.data.map(item=><tr key={item.id} tabIndex={0} onClick={()=>router.push(`/admin/feedback/${item.id}`)} onKeyDown={event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();router.push(`/admin/feedback/${item.id}`)}}}><td><strong>{item.bookingReference}</strong><small>{item.respondentName}</small></td><td><strong>{item.driver.displayName}</strong><small>{item.driver.sourceType==="AGENCY"?"Agency":`Outsourced · ${item.driver.vendorName||"Vendor unavailable"}`}</small></td><td>{formatInTimezone(item.submittedAt,response.meta.timezone)}</td><td><StatusBadge label={item.submissionMode==="OFFLINE_SYNC"?"Offline sync":"Online"} tone={item.submissionMode==="OFFLINE_SYNC"?"warning":"neutral"}/></td><td><ReviewBadge state={item.reviewState}/></td><td>{scoreLabel(item.overallScore)}</td></tr>)}</tbody></table></div><div className="feedback-cards">{response.data.map(item=><button className="card feedback-card" key={item.id} onClick={()=>router.push(`/admin/feedback/${item.id}`)}><span><strong>{item.bookingReference}</strong><small>{item.respondentName}</small></span><span>{item.driver.displayName}<small>{item.driver.sourceType==="OUTSOURCED"?item.driver.vendorName:"Agency driver"}</small></span><span>{formatInTimezone(item.submittedAt,response.meta.timezone)}</span><span className="trip-actions"><ReviewBadge state={item.reviewState}/><StatusBadge label={item.submissionMode==="OFFLINE_SYNC"?"Offline sync":"Online"}/></span><strong>{scoreLabel(item.overallScore)}</strong></button>)}</div></>}
