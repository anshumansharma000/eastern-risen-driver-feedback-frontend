...
import type { AdminFeedbackSummary, AdminFeedbackView, FeedbackListMeta, PaginatedResponse, QuestionCategory } from "@/lib/contracts";
...
const driverCategories:QuestionCategory[]=["OVERALL_EXPERIENCE","DRIVING_SAFETY","PUNCTUALITY","CLEANLINESS","PROFESSIONALISM","VEHICLE_CONDITION","CUSTOM"];
const companyCategories:QuestionCategory[]=["ARRIVAL_EXPERIENCE","TOUR_EXPERIENCE","TOUR_COORDINATION","CUSTOM"];
...
export function AdminFeedbackList(){
  ...
  const active=Object.entries(filters).some(([key,value])=>!["page","pageSize","view"].includes(key)&&value!==undefined);
  const thresholdMissing=error?.code==="NEGATIVE_FEEDBACK_THRESHOLD_REQUIRED";
  return <><div className="page-header"><div><p className="eyebrow">Immutable passenger records</p><h1>{view==="DRIVER"?"Driver feedback":"Company feedback"}</h1><p>{view==="DRIVER"?"Review answers about drivers, vehicles, safety, and service.":"Review arrival, booking, tour coordination, and overall tour experience."}</p></div><button className="button button-secondary" disabled={loading} onClick={()=>void load()}>Refresh</button></div>
    <div className="toolbar feedback-view-switcher"><label className="field"><span>Feedback view</span><select className="select" value={view} onChange={(event)=>search.update({view:event.target.value,category:null,driverId:null,driverSource:null,vendorId:null},true)}><option value="DRIVER">Driver feedback</option><option value="COMPANY">Company feedback</option></select></label></div>
    <section className="card card-pad feedback-filters" aria-label="Feedback filters">
      ...
      {view==="DRIVER"&&<><Filter label="Driver source" value={filters.driverSource||""} onChange={v=>update("driverSource",v)} options={[["AGENCY","Agency"],["OUTSOURCED","Outsourced"]]}/><label className="field"><span>Driver ID</span><input className="input" value={filters.driverId||""} onChange={e=>update("driverId",e.target.value)} placeholder="UUID"/></label><label className="field"><span>Vendor ID</span><input className="input" value={filters.vendorId||""} onChange={e=>update("vendorId",e.target.value)} placeholder="UUID"/></label></>}
      ...
      {active&&<button className="button button-secondary" onClick={()=>{window.history.pushState(null,"",`${window.location.pathname}?view=${view}`);window.dispatchEvent(new PopStateEvent("popstate"))}}>Clear filters</button>}
    </section>
    ...
    {response&&response.data.length>0&&<><p className="trip-meta feedback-date-basis">Times use {response.meta.timezone}; scores include only {view==="DRIVER"?"driver":"company"} feedback answers.</p><div className="feedback-table-wrap"><table className="feedback-table"><thead><tr><th>Booking / respondent</th><th>{view==="DRIVER"?"Driver":"Feedback sections"}</th><th>Submitted</th><th>Mode</th><th>Review</th><th>Score</th></tr></thead><tbody>{response.data.map(item=>{const detailPath=`/admin/feedback/detail?feedbackId=${encodeURIComponent(item.id)}&view=${view}`;return <tr key={item.id} tabIndex={0} onClick={()=>router.push(detailPath)} onKeyDown={event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();router.push(detailPath)}}}><td><strong>{item.bookingReference}</strong><small>{item.respondentName}</small></td><td>{view==="DRIVER"?<><strong>{item.driver.displayName}</strong><small>{item.driver.sourceType==="AGENCY"?"Agency":`Outsourced · ${item.driver.vendorName||"Vendor unavailable"}`}</small></>:<CompanyFeedbackSections/>}</td><td>{formatInTimezone(item.submittedAt,response.meta.timezone)}</td><td><StatusBadge label={item.submissionMode==="OFFLINE_SYNC"?"Offline sync":"Online"} tone={item.submissionMode==="OFFLINE_SYNC"?"warning":"neutral"}/></td><td><ReviewBadge state={item.reviewState}/></td><td>{scoreLabel(item.overallScore)}</td></tr>})}</tbody></table></div><div className="feedback-cards">{response.data.map(item=>{const detailPath=`/admin/feedback/detail?feedbackId=${encodeURIComponent(item.id)}&view=${view}`;return <button className="card feedback-card" key={item.id} onClick={()=>router.push(detailPath)}><span><strong>{item.bookingReference}</strong><small>{item.respondentName}</small></span><span>{view==="DRIVER"?<>{item.driver.displayName}<small>{item.driver.sourceType==="OUTSOURCED"?item.driver.vendorName:"Agency driver"}</small></>:<CompanyFeedbackSections/>}</span><span>{formatInTimezone(item.submittedAt,response.meta.timezone)}</span><span className="trip-actions"><ReviewBadge state={item.reviewState}/><StatusBadge label={item.submissionMode==="OFFLINE_SYNC"?"Offline sync":"Online"}/></span><strong>{scoreLabel(item.overallScore)}</strong></button>})}</div></>}
...
function CompanyFeedbackSections(){return <><strong>Arrival and booking experience</strong><small>Tour coordination and experience</small></>}
