...
export function AppShell({ role, children }: { role: "driver" | "admin"; children: ReactNode }) {
  ...
  useEffect(()=>{
    ...
    const close=(event:PointerEvent)=>{if(!profileMenuRef.current?.contains(event.target as Node))setProfileMenuOpen(false)};
    const escape=(event:KeyboardEvent)=>{if(event.key==="Escape")setProfileMenuOpen(false)};
  ...
  return <div className={`app-layout ${role==="driver"?"driver-shell":"admin-shell"}`}>
    ...
    <div className="app-main"><header className="topbar">
      <span className="mobile-brand">{driverContextScreen?<Link className="mobile-back" href="/driver" aria-label="Back to journeys"><span aria-hidden="true">←</span><strong>Journeys</strong></Link>:<Brand compact href={role==="driver"?"/driver":"/"} />}</span>
      <span className="eyebrow workspace-label">{role === "driver" ? "Driver workspace" : "Operations workspace"}</span>
      <span className="account">{principal?.displayName && <strong>{principal.displayName}</strong>}<span className="status status-success">Secure session</span></span>
      {role==="driver"?<div className="driver-profile-menu" ref={profileMenuRef}>
        <button className="driver-profile-button" type="button" aria-label="Open account menu" aria-haspopup="dialog" aria-expanded={profileMenuOpen} aria-controls="driver-account-menu" onClick={()=>setProfileMenuOpen(current=>!current)}>{initials}</button>
        {profileMenuOpen&&<div className="driver-profile-popover" id="driver-account-menu" role="dialog" aria-label="Driver account">
          <div className="driver-profile-summary"><span className="driver-profile-avatar" aria-hidden="true">{initials}</span><span><strong>{principal?.displayName||"Driver account"}</strong><small>Secure driver session</small></span></div>
          <Link href="/driver/profile" onClick={()=>setProfileMenuOpen(false)}>View profile</Link>
          <button type="button" onClick={logout}>Sign out</button>
        </div>}
      </div>:<button className="mobile-menu-button" type="button" aria-expanded={mobileNavOpen} aria-controls="mobile-navigation" onClick={()=>setMobileNavOpen(current=>!current)}>{mobileNavOpen?"Close":"Menu"}</button>}
    </header>
...
function DriverNavIcon({name}:{name:string}){
  return <span className={`driver-nav-icon driver-nav-icon-${name}`} aria-hidden="true"><i/><i/><i/></span>;
}
