...
export function TripCard({ trip, allowHandoff = true }: { trip: Trip; allowHandoff?: boolean }) {
  const router=useRouter(); const [confirm,setConfirm]=useState(false); const [busy,setBusy]=useState(false); const [error,setError]=useState<ApiErrorPresentation|null>(null); const state=tripStatus[trip.status];
  ...
  return <article className="card trip-card"><div className="trip-card-head"><div><span className="eyebrow">{tripSource[trip.creationSource]}</span><h3>{trip.booking.bookingReference}</h3><span className="trip-meta">{trip.booking.passengerName} · {formatTripRange(trip.scheduledAt,trip.scheduledEndAt)} · {trip.vehicle.displayName} · {trip.vehicle.registrationNumber}</span></div><StatusBadge label={state.label} tone={state.tone}/></div>
    ...
    {error&&<ErrorAlert {...error}/>}<div className="trip-actions"><Link className="button button-secondary" href={`/driver/trips/detail?tripId=${encodeURIComponent(trip.id)}`}>View trip</Link>{allowHandoff&&(trip.status==="READY"||trip.status==="FEEDBACK_STARTED")&&<button className="button" onClick={()=>setConfirm(true)}>Start passenger handoff</button>}{allowHandoff&&(trip.status==="READY"||trip.status==="FEEDBACK_STARTED")&&<ShareFeedbackOnWhatsAppAction tripId={trip.id} audience="driver" recipientName={trip.booking.passengerName}/>} {allowHandoff&&(trip.status==="READY"||trip.status==="FEEDBACK_STARTED")&&<ShareFeedbackLinkAction tripId={trip.id} audience="driver"/>}</div>
