...
export function TripCard({ trip, allowHandoff = true }: { trip: Trip; allowHandoff?: boolean }) {
  ...
  return <article className="card trip-card"><div className="trip-card-head"><div><span className="eyebrow">{tripSource[trip.creationSource]}</span><h3>{trip.bookingReference}</h3><span className="trip-meta">{formatIndiaDate(trip.scheduledAt)} · {trip.vehicle.displayName} · {trip.vehicle.registrationNumber}</span></div><StatusBadge label={state.label} tone={state.tone}/></div>
