...
export function ErrorAlert({ message, requestId }: { message: string; requestId?: string }) { return <div className="alert" role="alert"><strong>We couldn’t complete that.</strong><div>{message}</div>{requestId && <small>Request ID: {requestId}</small>}</div>; }
